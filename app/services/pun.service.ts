...
export class PunService {
  suggestKeywords(partial: string): Observable<string[]> {
    const keywords = suggestKeywords(partial);
    return Observable.of(keywords);
  }

  getPuns(keywords: string[]): Observable<Pun[]> {
    const puns = getPuns(keywords);
    return Observable.of(puns);
  }
}

function suggestKeywords(partial: string) {
  if (!partial) {
    return [];
  }

  return Object.keys(mocks.keywords)
    .filter(keyword => keyword.indexOf(partial.toLowerCase()) === 0)
}
...
function getPuns(keywords) {
  ...
  const found = keywords.reduce((found, keyword) => {
    const ids = mocks.keywords[keyword];
    if (ids) {
